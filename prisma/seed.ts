...
async function main() {
  ...
  await prisma.serverRecord.upsert({
    where: { record_worldId_timestamp: { record: 0, worldId: 0, timestamp: 0 } },
    update: {},
  ...
  const adminName = process.env.SEED_ADMIN_NAME ?? "tiagopersch";
  const adminPassword = process.env.SEED_ADMIN_PASSWORD ?? "AylaAyuni0717@";
  ...
  console.log(`  Conta admin: name="${adminName}" senha="${adminPassword}" (group_id=6)`);
}
