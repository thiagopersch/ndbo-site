...
import { SPELL_GROUP_OPTIONS, SPELL_KINDS } from "@/lib/validations/admin/spell";
...
export default function AdminSpellsPage() {
  ...
  const { data, isLoading, isValidating, mutate } = useSWR<PaginatedResult<SpellRow>>(
    `/api/admin/spells?${table.buildQueryParams().toString()}`,
    fetcher
  );

  const { data: levelsData } = useSWR<{ levels: number[] }>("/api/admin/spells/levels", fetcher);
  ...
  const filterFields: FilterFieldConfig[] = [
    ...
    },
    {
      ...
      label: "Level",
      type: "select",
      options: (levelsData?.levels ?? []).map((level) => ({ value: String(level), label: String(level) })),
    },
    {
      key: "group",
      label: "Grupo",
      type: "multi-select",
      options: SPELL_GROUP_OPTIONS.map((group) => ({ value: group, label: group })),
    },
    {
      key: "vocationIds",
      label: "Vocações",
  ...
  async function handleDelete(id: number) {
    const response = await fetch(`/api/admin/spells/${id}`, { method: "DELETE" });
  ...
  const columns: ColumnDef<SpellRow>[] = [
    { accessorKey: "id", header: "ID" },
    {
      ...
      header: "Tipo",
      cell: ({ row }) => (
        <Badge variant={KIND_BADGE_VARIANT[row.original.kind] ?? "secondary"}>{row.original.kind}</Badge>
      ),
    },
    ...
    { accessorKey: "mana", header: "Mana" },
    {
      ...
      cell: ({ row }) => {
        ...
        const names = row.original.vocations.map((entry) => entry.vocation.name);
      ...
      <div className="flex flex-col gap-6">
        <div>
          ...
          <p className="text-muted-foreground">
            CRUD de feitiços exportável para o <code>spells.xml</code> (instant/rune/conjure).
          </p>
        </div>
          ...
          toolbar={
            <>
              <XmlImportDialog
                ...
                description={
                  <>
                    Envie um arquivo no formato do <code>spells.xml</code> do servidor. Spells com
                    dados inválidos são ignoradas e reportadas ao final.
                  </>
                }
              ...
              <Button nativeButton={false} render={<Link href="/admin/spells/new" />}>
